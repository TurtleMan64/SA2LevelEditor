...
std::list<TexturedModel*> CLEAR_SW::models;
CollisionModel* CLEAR_SW::cmBase;
...
CLEAR_SW::CLEAR_SW(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = CLEAR_SW::cmBase;
    collideModelTransformed = CLEAR_SW::cmBase->duplicateMe();
...
std::list<TexturedModel*>* CLEAR_SW::getModels()
{
    return &CLEAR_SW::models;
}

void CLEAR_SW::loadStaticModels()
{
    if (CLEAR_SW::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading CLEAR_SW static models...\n");
    #endif

    loadModel(&CLEAR_SW::models, "res/Models/LevelObjects/CannonsCore/", "CLEAR_SW");

    if (CLEAR_SW::cmBase == nullptr)
    {
        CLEAR_SW::cmBase = loadCollisionModel("res/Models/LevelObjects/CannonsCore/", "CLEAR_SW");
    }
}

void CLEAR_SW::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting CLEAR_SW static models...\n");
    #endif

    Entity::deleteModels(&CLEAR_SW::models);
    Entity::deleteCollisionModel(&CLEAR_SW::cmBase);
}

void CLEAR_SW::updateValue(int btnIndex)
{
    char buf[128];
...
void CLEAR_SW::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowLabels[ 8], "Var 1");
    SetWindowTextA(Global::windowLabels[ 9], "Var 2");
    SetWindowTextA(Global::windowLabels[10], "var 3");

    SetWindowTextA(Global::windowValues[ 0], std::to_string(ID).c_str());
    SetWindowTextA(Global::windowValues[ 1], "CLEAR_SW");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(rotationX).c_str());
...
void CLEAR_SW::fillData(char data[32])
{
    data[1] = (char)ID;
