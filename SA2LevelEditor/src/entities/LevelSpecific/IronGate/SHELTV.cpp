...
std::list<TexturedModel*> SHELTV::models;
CollisionModel* SHELTV::cmBase;
...
SHELTV::SHELTV(char data[32], bool /*useDefaultValues*/)
{
    ...
    collideModelOriginal = SHELTV::cmBase;
    collideModelTransformed = SHELTV::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);

    Maths::createTransformationMatrixZXY(&transformationMatrix, &position, 0, rotationY, 0, 1.0f, 1.0f, 1.0f);
    collideModelOriginal->transformModelZXY(collideModelTransformed, &position, 0, rotationY, 0, 1.0f, 1.0f, 1.0f);
}

bool SHELTV::isSA2Object()
{
    return true;
}
...
std::list<TexturedModel*>* SHELTV::getModels()
{
    return &SHELTV::models;
}

void SHELTV::loadStaticModels()
{
    if (SHELTV::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading SHELTV static models...\n");
    #endif

    loadModel(&SHELTV::models, "res/Models/LevelObjects/IronGate/", "SHELTV");

    if (SHELTV::cmBase == nullptr)
    {
        SHELTV::cmBase = loadCollisionModel("res/Models/LevelObjects/IronGate/", "SHELTV");
    }
}

void SHELTV::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting SHELTV static models...\n");
    #endif

    Entity::deleteModels(&SHELTV::models);
    Entity::deleteCollisionModel(&SHELTV::cmBase);
}

void SHELTV::updateValue(int btnIndex)
{
    char buf[128];
...
void SHELTV::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "SHELTH");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(rotationX).c_str());
...
void SHELTV::fillData(char data[32])
{
    data[1] = (char)ID;
