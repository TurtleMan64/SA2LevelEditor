...
std::list<TexturedModel*> WALL::models;
CollisionModel* WALL::cmBase;
...
WALL::WALL(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = WALL::cmBase;
    collideModelTransformed = WALL::cmBase->duplicateMe();
...
std::list<TexturedModel*>* WALL::getModels()
{
    return &WALL::models;
}

void WALL::loadStaticModels()
{
    if (WALL::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading WALL static models...\n");
    #endif

    loadModel(&WALL::models, "res/Models/LevelObjects/WeaponsBed/", "WALL");

    if (WALL::cmBase == nullptr)
    {
        WALL::cmBase = loadCollisionModel("res/Models/LevelObjects/WeaponsBed/", "WALL");
    }
}

void WALL::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting WALL static models...\n");
    #endif

    Entity::deleteModels(&WALL::models);
    Entity::deleteCollisionModel(&WALL::cmBase);
}

void WALL::updateValue(int btnIndex)
{
    char buf[128];
...
void WALL::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "WALL");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(rotationX).c_str());
...
void WALL::fillData(char data[32])
{
    data[1] = (char)ID;
