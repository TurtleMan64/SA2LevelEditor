...
std::list<TexturedModel*> FIG_ENE::models;
CollisionModel* FIG_ENE::cmBase;
...
FIG_ENE::FIG_ENE(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = FIG_ENE::cmBase;
    collideModelTransformed = FIG_ENE::cmBase->duplicateMe();
...
std::list<TexturedModel*>* FIG_ENE::getModels()
{
    return &FIG_ENE::models;
}

void FIG_ENE::loadStaticModels()
{
    if (FIG_ENE::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading FIG_ENE static models...\n");
    #endif

    loadModel(&FIG_ENE::models, "res/Models/LevelObjects/WeaponsBed/", "FIG_SHOUKO");

    if (FIG_ENE::cmBase == nullptr)
    {
        FIG_ENE::cmBase = loadCollisionModel("res/Models/LevelObjects/WeaponsBed/", "FIG_SHOUKO");
    }
}

void FIG_ENE::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting FIG_ENE static models...\n");
    #endif

    Entity::deleteModels(&FIG_ENE::models);
    Entity::deleteCollisionModel(&FIG_ENE::cmBase);
}

void FIG_ENE::updateValue(int btnIndex)
{
    char buf[128];
...
void FIG_ENE::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "FIG_ENE");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(rotationX).c_str());
...
void FIG_ENE::fillData(char data[32])
{
    data[1] = (char)ID;
