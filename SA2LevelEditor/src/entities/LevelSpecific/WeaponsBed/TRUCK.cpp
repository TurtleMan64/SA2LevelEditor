...
std::list<TexturedModel*> TRUCK::models;
CollisionModel* TRUCK::cmBase;
...
TRUCK::TRUCK(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = TRUCK::cmBase;
    collideModelTransformed = TRUCK::cmBase->duplicateMe();
...
std::list<TexturedModel*>* TRUCK::getModels()
{
    return &TRUCK::models;
}

void TRUCK::loadStaticModels()
{
    if (TRUCK::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading TRUCK static models...\n");
    #endif

    loadModel(&TRUCK::models, "res/Models/LevelObjects/WeaponsBed/", "TRUCK");

    if (TRUCK::cmBase == nullptr)
    {
        TRUCK::cmBase = loadCollisionModel("res/Models/LevelObjects/WeaponsBed/", "TRUCK");
    }
}

void TRUCK::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting TRUCK static models...\n");
    #endif

    Entity::deleteModels(&TRUCK::models);
    Entity::deleteCollisionModel(&TRUCK::cmBase);
}

void TRUCK::updateValue(int btnIndex)
{
    char buf[128];
...
void TRUCK::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "TRUCK");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(rotationX).c_str());
...
void TRUCK::fillData(char data[32])
{
    data[1] = (char)ID;
