...
std::list<TexturedModel*> AIRCONT::models;
CollisionModel* AIRCONT::cmBase;
...
AIRCONT::AIRCONT(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = AIRCONT::cmBase;
    collideModelTransformed = AIRCONT::cmBase->duplicateMe();
...
std::list<TexturedModel*>* AIRCONT::getModels()
{
    return &AIRCONT::models;
}

void AIRCONT::loadStaticModels()
{
    if (AIRCONT::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading AIRCONT static models...\n");
    #endif

    loadModel(&AIRCONT::models, "res/Models/LevelObjects/MeteorHerd/", "AIRCONT");

    if (AIRCONT::cmBase == nullptr)
    {
        AIRCONT::cmBase = loadCollisionModel("res/Models/LevelObjects/MeteorHerd/", "AIRCONT");
    }
}

void AIRCONT::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting AIRCONT static models...\n");
    #endif

    Entity::deleteModels(&AIRCONT::models);
    Entity::deleteCollisionModel(&AIRCONT::cmBase);
}

void AIRCONT::updateValue(int btnIndex)
{
    char buf[128];
...
void AIRCONT::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "AIRCONT");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(rotationX).c_str());
...
void AIRCONT::fillData(char data[32])
{
    data[1] = (char)ID;
