...
std::list<TexturedModel*> CONTWOOD::models;
CollisionModel* CONTWOOD::cmBase;
...
CONTWOOD::CONTWOOD(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = CONTWOOD::cmBase;
    collideModelTransformed = CONTWOOD::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelY();
}

bool CONTWOOD::isSA2Object()
{
    return true;
}
...
std::list<TexturedModel*>* CONTWOOD::getModels()
{
    return &CONTWOOD::models;
}

void CONTWOOD::loadStaticModels()
{
    if (CONTWOOD::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading CONTWOOD static models...\n");
    #endif

    loadModel(&CONTWOOD::models, "res/Models/GlobalObjects/Box/CONTWOOD/", "CONTWOOD");

    if (CONTWOOD::cmBase == nullptr)
    {
        CONTWOOD::cmBase = loadCollisionModel("res/Models/GlobalObjects/Box/CONTWOOD/", "CONTWOOD");
    }
}

void CONTWOOD::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting CONTWOOD static models...\n");
    #endif

    Entity::deleteModels(&CONTWOOD::models);
    Entity::deleteCollisionModel(&CONTWOOD::cmBase);
}

void CONTWOOD::updateValue(int btnIndex)
{
    char buf[128];
...
void CONTWOOD::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "CONTWOOD");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    ...
    SetWindowTextA(Global::windowDescriptions[ 1], "CONTWOOD");
...
void CONTWOOD::fillData(char data[32])
{
    data[1] = (char)ID;
