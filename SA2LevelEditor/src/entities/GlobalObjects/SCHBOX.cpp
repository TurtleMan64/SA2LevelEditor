...
std::list<TexturedModel*> SCHBOX::models;
CollisionModel* SCHBOX::cmBase;
...
SCHBOX::SCHBOX(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = SCHBOX::cmBase;
    collideModelTransformed = SCHBOX::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelYXZ();
}

bool SCHBOX::isSA2Object()
{
    return true;
}
...
std::list<TexturedModel*>* SCHBOX::getModels()
{
    return &SCHBOX::models;
}

void SCHBOX::loadStaticModels()
{
    if (SCHBOX::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading SCHBOX static models...\n");
    #endif

    loadModel(&SCHBOX::models, "res/Models/GlobalObjects/Monitor/", "Monitor");

    if (SCHBOX::cmBase == nullptr)
    {
        SCHBOX::cmBase = loadCollisionModel("res/Models/GlobalObjects/Monitor/", "Monitor");
    }
}

void SCHBOX::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting SCHBOX static models...\n");
    #endif

    Entity::deleteModels(&SCHBOX::models);
    Entity::deleteCollisionModel(&SCHBOX::cmBase);
}

void SCHBOX::updateValue(int btnIndex)
{
    char buf[128];
...
void SCHBOX::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "SCHBOX");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(rotationX).c_str());
...
void SCHBOX::fillData(char data[32])
{
    data[1] = (char)ID;
