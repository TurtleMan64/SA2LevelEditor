...
std::list<TexturedModel*> PAWNGUN::models;
CollisionModel* PAWNGUN::cmBase = nullptr;
...
PAWNGUN::PAWNGUN(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = PAWNGUN::cmBase;
    collideModelTransformed = PAWNGUN::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelYXZ();
}

bool PAWNGUN::isSA2Object()
{
    return true;
}
...
std::list<TexturedModel*>* PAWNGUN::getModels()
{
    return &PAWNGUN::models;
}

void PAWNGUN::loadStaticModels()
{
    if (PAWNGUN::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading PAWNGUN static models...\n");
    #endif

    loadModel(&PAWNGUN::models, "res/Models/GlobalObjects/Heroes/EggPawnGun/", "PawnArmless");

    if (PAWNGUN::cmBase == nullptr)
    {
        PAWNGUN::cmBase = loadCollisionModel("res/Models/GlobalObjects/Heroes/EggPawnGun/", "PawnArmless");
    }
}

void PAWNGUN::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting PAWNGUN static models...\n");
    #endif

    Entity::deleteModels(&PAWNGUN::models);
    Entity::deleteCollisionModel(&PAWNGUN::cmBase);
}

void PAWNGUN::updateValue(int btnIndex)
{
    char buf[128];
...
void PAWNGUN::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "PAWNGUN");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(Maths::bamsToDeg(rotationX)).c_str());
    ...
    SetWindowTextA(Global::windowDescriptions[ 1], "Egg Pawn Gun");
...
void PAWNGUN::fillData(char data[32])
{
    data[1] = (char)ID;
...
std::string PAWNGUN::toSabString()
{
    Vector3f dir(1, 0, 0);
