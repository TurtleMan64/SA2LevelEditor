...
std::list<TexturedModel*> PAWN::models;
CollisionModel* PAWN::cmBase = nullptr;
...
PAWN::PAWN(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = PAWN::cmBase;
    collideModelTransformed = PAWN::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelYXZ();
}

bool PAWN::isSA2Object()
{
    return true;
}
...
std::list<TexturedModel*>* PAWN::getModels()
{
    return &PAWN::models;
}

void PAWN::loadStaticModels()
{
    if (PAWN::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading PAWN static models...\n");
    #endif

    loadModel(&PAWN::models, "res/Models/GlobalObjects/Heroes/EggPawn/", "EggPawn");

    if (PAWN::cmBase == nullptr)
    {
        PAWN::cmBase = loadCollisionModel("res/Models/GlobalObjects/Heroes/EggPawn/", "EggPawn");
    }
}

void PAWN::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting PAWN static models...\n");
    #endif

    Entity::deleteModels(&PAWN::models);
    Entity::deleteCollisionModel(&PAWN::cmBase);
}

void PAWN::updateValue(int btnIndex)
{
    char buf[128];
...
void PAWN::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "PAWN");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(Maths::bamsToDeg(rotationX)).c_str());
    ...
    SetWindowTextA(Global::windowDescriptions[ 1], "Egg Pawn");
...
void PAWN::fillData(char data[32])
{
    data[1] = (char)ID;
