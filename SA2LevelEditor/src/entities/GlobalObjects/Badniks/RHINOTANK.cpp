...
CollisionModel* RHINOTANK::cmBase = nullptr;
...
RHINOTANK::RHINOTANK(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = RHINOTANK::cmBase;
    collideModelTransformed = RHINOTANK::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelYXZ();
}

bool RHINOTANK::isSA2Object()
{
    return true;
}
...
void RHINOTANK::loadStaticModels()
{
    ...
    loadModel(&RHINOTANK::models, "res/Models/GlobalObjects/Badniks/Rhinotank/", "Rhinotank");

    if (RHINOTANK::cmBase == nullptr)
    {
        RHINOTANK::cmBase = loadCollisionModel("res/Models/GlobalObjects/Badniks/Rhinotank/", "Rhinotank");
    }
}
...
void RHINOTANK::deleteStaticModels()
{
    ...
    Entity::deleteCollisionModel(&RHINOTANK::cmBase);
}

void RHINOTANK::updateValue(int btnIndex)
{
    char buf[128];
...
void RHINOTANK::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "RHINOTANK");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(Maths::bamsToDeg(rotationX)).c_str());
    ...
    SetWindowTextA(Global::windowDescriptions[ 1], "Rhino Tank");
...
void RHINOTANK::fillData(char data[32])
{
    data[1] = (char)ID;
...
std::string RHINOTANK::toSabString()
{
    Vector3f dir(1, 0, 0);
