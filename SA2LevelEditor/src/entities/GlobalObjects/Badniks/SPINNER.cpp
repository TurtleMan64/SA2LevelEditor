...
std::list<TexturedModel*> SPINNER::models;
CollisionModel* SPINNER::cmBase = nullptr;
...
SPINNER::SPINNER(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = SPINNER::cmBase;
    collideModelTransformed = SPINNER::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelYXZ();
}

bool SPINNER::isSA2Object()
{
    return true;
}
...
{
    return &SPINNER::models;
}

void SPINNER::loadStaticModels()
{
    if (SPINNER::models.size() > 0)
    {
        return;
    }
    ...
    loadModel(&SPINNER::models, "res/Models/GlobalObjects/Badniks/Spinner/", "Body");

    if (SPINNER::cmBase == nullptr)
    {
        SPINNER::cmBase = loadCollisionModel("res/Models/GlobalObjects/Badniks/Spinner/", "Body");
    }
}
...
void SPINNER::deleteStaticModels()
{
    ...
    Entity::deleteModels(&SPINNER::models);
    Entity::deleteCollisionModel(&SPINNER::cmBase);
}

void SPINNER::updateValue(int btnIndex)
{
    char buf[128];
...
void SPINNER::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "SPINNER");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[ 5], std::to_string(Maths::bamsToDeg(rotationX)).c_str());
    ...
    SetWindowTextA(Global::windowDescriptions[ 1], "Spinner");
...
void SPINNER::fillData(char data[32])
{
    data[1] = (char)ID;
...
std::string SPINNER::toSabString()
{
    return "123 " + 
