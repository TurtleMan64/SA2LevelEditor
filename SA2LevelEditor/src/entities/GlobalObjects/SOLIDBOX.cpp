...
std::list<TexturedModel*> SOLIDBOX::models;
CollisionModel* SOLIDBOX::cmBase;
...
SOLIDBOX::SOLIDBOX(char data[32], bool useDefaultValues)
{
    ...
    updateTransformationMatrixY();

    collideModelOriginal = SOLIDBOX::cmBase;
    collideModelTransformed = SOLIDBOX::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelY();
}

bool SOLIDBOX::isSA2Object()
{
    return true;
}
...
std::list<TexturedModel*>* SOLIDBOX::getModels()
{
    return &SOLIDBOX::models;
}

void SOLIDBOX::loadStaticModels()
{
    if (SOLIDBOX::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading SOLIDBOX static models...\n");
    #endif

    loadModel(&SOLIDBOX::models, "res/Models/GlobalObjects/Box/SOLIDBOX/", "SOLIDBOX");

    if (SOLIDBOX::cmBase == nullptr)
    {
        SOLIDBOX::cmBase = loadCollisionModel("res/Models/GlobalObjects/Box/SOLIDBOX/", "SOLIDBOX");
    }
}

void SOLIDBOX::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting SOLIDBOX static models...\n");
    #endif

    Entity::deleteModels(&SOLIDBOX::models);
    Entity::deleteCollisionModel(&SOLIDBOX::cmBase);
}

void SOLIDBOX::updateValue(int btnIndex)
{
    char buf[128];
...
void SOLIDBOX::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowLabels[ 5], "Unknown");
    SetWindowTextA(Global::windowLabels[ 6], "Rotation Y");
    SetWindowTextA(Global::windowLabels[ 7], "Unknown");
    ...
    SetWindowTextA(Global::windowValues[ 1], "SOLIDBOX");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    ...
    SetWindowTextA(Global::windowDescriptions[ 1], "SOLIDBOX");
...
void SOLIDBOX::fillData(char data[32])
{
    data[1] = (char)ID;
