...
std::list<TexturedModel*> ORI::models;
CollisionModel* ORI::cmBase;
...
ORI::ORI(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = ORI::cmBase;
    collideModelTransformed = ORI::cmBase->duplicateMe();
    collideModelTransformed->parent = this;
    CollisionChecker::addCollideModel(collideModelTransformed);
    updateCollisionModelY();
}

bool ORI::isSA2Object()
{
    return true;
}
...
std::list<TexturedModel*>* ORI::getModels()
{
    return &ORI::models;
}

void ORI::loadStaticModels()
{
    if (ORI::models.size() > 0)
    {
        return;
    }

    #ifdef DEV_MODE
    std::fprintf(stdout, "Loading ORI static models...\n");
    #endif

    loadModel(&ORI::models, "res/Models/GlobalObjects/Box/ORI/", "ORI");

    if (ORI::cmBase == nullptr)
    {
        ORI::cmBase = loadCollisionModel("res/Models/GlobalObjects/Box/ORI/", "ORI");
    }
}

void ORI::deleteStaticModels()
{
    #ifdef DEV_MODE
    std::fprintf(stdout, "Deleting ORI static models...\n");
    #endif

    Entity::deleteModels(&ORI::models);
    Entity::deleteCollisionModel(&ORI::cmBase);
}

void ORI::updateValue(int btnIndex)
{
    char buf[128];
...
void ORI::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[ 1], "ORI");
    SetWindowTextA(Global::windowValues[ 2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[ 3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[ 4], std::to_string(position.z).c_str());
    ...
    SetWindowTextA(Global::windowDescriptions[ 1], "ORI");
...
void ORI::fillData(char data[32])
{
    data[1] = (char)ID;
